...
_start:
    ; Print banner — write(fd=1, buf, len)
    mov rax, SYS_WRITE
    mov rdi, 1              ; fd = stdout
    lea rsi, [rel banner]
    mov rdx, banner_len
    ...
    ; sockfd = socket() — legacy TCP socket for now
    mov rax, SYS_SOCKET
    xor rdi, rdi            ; domain (ignored in legacy)
    xor rsi, rsi            ; type
    ...
    mov rax, SYS_WRITE
    mov rdi, 1
    lea rsi, [rel recv_msg]
    mov rdx, recv_msg_len
    syscall
