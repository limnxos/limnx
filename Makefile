...
build/user/s75test.o: user/s75test.c user/libc/libc.h
	@mkdir -p $(dir $@)
	$(CC) $(USER_CFLAGS) -c $< -o $@

build/user/s75test.elf: build/user/s75test.o $(LIBC_OBJS) user/libc/linker.ld
	$(LD) -nostdlib -static -T user/libc/linker.ld $(LIBC_OBJS) $< -o $@

build/user/s76test.o: user/s76test.c user/libc/libc.h
	@mkdir -p $(dir $@)
	$(CC) $(USER_CFLAGS) -c $< -o $@

build/user/s76test.elf: build/user/s76test.o $(LIBC_OBJS) user/libc/linker.ld
	$(LD) -nostdlib -static -T user/libc/linker.ld $(LIBC_OBJS) $< -o $@

build/user/s77test.o: user/s77test.c user/libc/libc.h
	@mkdir -p $(dir $@)
	$(CC) $(USER_CFLAGS) -c $< -o $@

build/user/s77test.elf: build/user/s77test.o $(LIBC_OBJS) user/libc/linker.ld
	$(LD) -nostdlib -static -T user/libc/linker.ld $(LIBC_OBJS) $< -o $@

build/user/s78test.o: user/s78test.c user/libc/libc.h
	@mkdir -p $(dir $@)
	$(CC) $(USER_CFLAGS) -c $< -o $@

build/user/s78test.elf: build/user/s78test.o $(LIBC_OBJS) user/libc/linker.ld
	$(LD) -nostdlib -static -T user/libc/linker.ld $(LIBC_OBJS) $< -o $@
