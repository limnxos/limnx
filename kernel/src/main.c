...
void kmain(void) {
    ...
    serial_puts("[test] Loading s49test.elf...\n");
    {
        process_t *s49_proc = load_and_run_elf("/s49test.elf");
        if (s49_proc) {
            serial_printf("[test] s49test.elf spawned (pid %lu)\n",
                          s49_proc->pid);
            process_reap(s49_proc);
            serial_puts("[test] s49test.elf completed\n");
        } else {
            serial_puts("[test] s49test.elf not found or failed to load\n");
        }
    }

    serial_puts("\n========================================\n");
    serial_puts("  Stage 49 init complete\n");
    serial_puts("========================================\n");

    /* --- Stage 50 init --- */
    serial_puts("\n--- Stage 50 init ---\n\n");

    serial_puts("[test] Loading s50test.elf...\n");
    {
        process_t *s50_proc = load_and_run_elf("/s50test.elf");
        if (s50_proc) {
            serial_printf("[test] s50test.elf spawned (pid %lu)\n",
                          s50_proc->pid);
            process_reap(s50_proc);
            serial_puts("[test] s50test.elf completed\n");
        } else {
            serial_puts("[test] s50test.elf not found or failed to load\n");
        }
    }

    serial_puts("\n========================================\n");
    serial_puts("  Stage 50 init complete\n");
    serial_puts("========================================\n");

    /* --- Stage 51 init --- */
    serial_puts("\n--- Stage 51 init ---\n\n");

    serial_puts("[test] Loading s51test.elf...\n");
    {
        process_t *s51_proc = load_and_run_elf("/s51test.elf");
        if (s51_proc) {
            serial_printf("[test] s51test.elf spawned (pid %lu)\n",
                          s51_proc->pid);
            process_reap(s51_proc);
            serial_puts("[test] s51test.elf completed\n");
        } else {
            serial_puts("[test] s51test.elf not found or failed to load\n");
        }
    }

    serial_puts("\n========================================\n");
    serial_puts("  Stage 51 init complete\n");
    serial_puts("========================================\n");

    /* --- Stage 52 init --- */
    serial_puts("\n--- Stage 52 init ---\n\n");

    serial_puts("[test] Loading s52test.elf...\n");
    {
        process_t *s52_proc = load_and_run_elf("/s52test.elf");
        if (s52_proc) {
            serial_printf("[test] s52test.elf spawned (pid %lu)\n",
                          s52_proc->pid);
            process_reap(s52_proc);
            serial_puts("[test] s52test.elf completed\n");
        } else {
            serial_puts("[test] s52test.elf not found or failed to load\n");
        }
    }
    ...
    serial_puts("[test] Loading s53test.elf...\n");
    {
        process_t *s53_proc = load_and_run_elf("/s53test.elf");
        if (s53_proc) {
            serial_printf("[test] s53test.elf spawned (pid %lu)\n",
                          s53_proc->pid);
            process_reap(s53_proc);
            serial_puts("[test] s53test.elf completed\n");
        } else {
            serial_puts("[test] s53test.elf not found or failed to load\n");
        }
    }
    ...
    serial_puts("[test] Loading s54test.elf...\n");
    {
        process_t *s54_proc = load_and_run_elf("/s54test.elf");
        if (s54_proc) {
            serial_printf("[test] s54test.elf spawned (pid %lu)\n",
                          s54_proc->pid);
            process_reap(s54_proc);
            serial_puts("[test] s54test.elf completed\n");
        } else {
            serial_puts("[test] s54test.elf not found or failed to load\n");
        }
    }
    ...
    serial_puts("[test] Loading s55test.elf...\n");
    {
        process_t *s55_proc = load_and_run_elf("/s55test.elf");
        if (s55_proc) {
            serial_printf("[test] s55test.elf spawned (pid %lu)\n",
                          s55_proc->pid);
            process_reap(s55_proc);
            serial_puts("[test] s55test.elf completed\n");
        } else {
            serial_puts("[test] s55test.elf not found or failed to load\n");
        }
    }
    ...
    serial_puts("[test] Loading s57test.elf...\n");
    {
        process_t *s57_proc = load_and_run_elf("/s57test.elf");
        if (s57_proc) {
            serial_printf("[test] s57test.elf spawned (pid %lu)\n",
                          (unsigned long)s57_proc->pid);
            process_reap(s57_proc);
            serial_puts("[test] s57test.elf completed\n");
        } else {
            serial_puts("[test] s57test.elf not found or failed to load\n");
        }
    }

    serial_puts("\n========================================\n");
    serial_puts("  Stage 57 init complete\n");
    serial_puts("========================================\n");

    /* --- Stage 58 init --- */
    serial_puts("\n--- Stage 58 init ---\n\n");

    serial_puts("[test] Loading s58test.elf...\n");
    {
        process_t *s58_proc = load_and_run_elf("/s58test.elf");
        if (s58_proc) {
            serial_printf("[test] s58test.elf spawned (pid %lu)\n",
                          (unsigned long)s58_proc->pid);
            process_reap(s58_proc);
            serial_puts("[test] s58test.elf completed\n");
        } else {
            serial_puts("[test] s58test.elf not found or failed to load\n");
        }
    }
